...
function design_coupling_fix_initial_guess_all()
    ...
    for idx = 1:length(x0)
        ...
        data_cmdx(idx, :) = cmdx;
        if idx<=length(x0)-1
        [xptmp, fptmp, eptmp, optmp] = fmincon(@(x) objfun_fix(x, vehicle, range, payload,idx,x0,len), x0(idx:idx+1), [], [], [], [], lb(idx:idx+1), ub(idx:idx+1), @(x) constr_fix(x, vehicle, range, payload, idx, xpdx(idx),x0,len), opt);
        [xmtmp, fmtmp, emtmp, omtmp] = fmincon(@(x) objfun_fix(x, vehicle, range, payload,idx,x0,len), x0(idx:idx+1), [], [], [], [], lb(idx:idx+1), ub(idx:idx+1), @(x) constr_fix(x, vehicle, range, payload, idx, xmdx(idx),x0,len), opt);
        end
        if idx==length(x0)
        [xptmp, fptmp, eptmp, optmp] = fmincon(@(x) objfun_fix(x, vehicle, range, payload,idx,x0,len), x0(idx:-(length(x0)-1):1), [], [], [], [], lb(idx:-(length(x0)-1):1), ub(idx:-(length(x0)-1):1), @(x) constr_fix(x, vehicle, range, payload, idx, xpdx(idx),x0,len), opt);
        [xmtmp, fmtmp, emtmp, omtmp] = fmincon(@(x) objfun_fix(x, vehicle, range, payload,idx,x0,len), x0(idx:-(length(x0)-1):1), [], [], [], [],lb(idx:-(length(x0)-1):1), ub(idx:-(length(x0)-1):1), @(x) constr_fix(x, vehicle, range, payload, idx, xmdx(idx),x0,len), opt);
        end
        i=1;
...
%      ax.YAxis.TickLabelColor = [0 0 0];
    ...
    function y = objfun_fix(x, vehicle, range, payload,idx,x0,len)
        if ~isequal(x,xLast) % Check if computation is necessary
            [myf,myc,myceq] = computePerformance_fix(x, vehicle, range, payload,idx,x0,len);
            xLast = x;
        end
        % Now compute objective function
        y = myf;
    ...
    function [c,ceq] = constr_fix(x, vehicle, range, payload, ifix, xfix,x0,len)
        if ~isequal(x,xLast) % Check if computation is necessary
            [myf,myc,myceq] = computePerformance_fix(x, vehicle, range, payload,ifix,x0,len);
            xLast = x;
        end
        myceq = x(1) - xfix;
        % Now compute constraint function
